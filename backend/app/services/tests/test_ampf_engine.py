...
Since the full import chain requires app.models.schemas (not present),
we mock the imports and test the pure computational functions directly.
"""
...
# Mock missing modules before importing ampf_engine
...
_mock_seg = types.ModuleType("app.services.segmentation_service")
_mock_seg.SegmentationService = MagicMock
sys.modules["app.services.segmentation_service"] = _mock_seg

# Now safe to import
backend_root = Path(__file__).resolve().parent.parent.parent.parent
sys.path.insert(0, str(backend_root))

from app.services.ampf_engine import (
    AMPFConfig,
    AMPFEngine,
    Candidate,
)
...
# Helper: create engine with mock service
...
def test_composite_confidence_formula(s_det, s_stab, s_bound):
    ...
    expected = 0.4 * s_det + 0.35 * s_stab + 0.25 * s_bound
    assert 0.0 <= expected <= 1.0
    assert abs(expected - (config.alpha * s_det + config.beta * s_stab + config.gamma * s_bound)) < 1e-9


# --- Property 5: Boundary score range [0, 1] ---

def _random_binary_mask(h=50, w=50, fill_ratio=0.3):
    mask = np.zeros((h, w), dtype=np.uint8)
    n = int(h * w * fill_ratio)
    indices = np.random.choice(h * w, size=n, replace=False)
...
def test_fusion_fallback_all_below_threshold():
    ...
    # Should fallback to best candidate
